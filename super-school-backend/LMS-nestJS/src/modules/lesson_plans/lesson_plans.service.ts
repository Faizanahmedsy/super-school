...
export class LessonPlanService {
    constructor(
        @InjectRepository(LessonPlans)
        private lessonPlansRepository: Repository<LessonPlans>,
    ...
    async create(createLessonPlanDto: CreateLessonPlanDto) {
        const newDivisionSubject = this.lessonPlansRepository.create(createLessonPlanDto);
        return await this.lessonPlansRepository.save(newDivisionSubject);
    }

    /*
     *   Check if the plan exists
     */
    async isExist(id: number): Promise<LessonPlans | null> {
        return await this.lessonPlansRepository.findOne({ where: { id } });
    }
    ...
    async findAll(reqQuery?: any) {
        ...
        const [subjects, totalCount] = await this.lessonPlansRepository.findAndCount({
            where: queryObject,
            relations: ["teacher", "grade", "master_subject", "term", "batch", "division", "subject", "creator"],
            select: {
                creator: {
                    id: true,
                    user_name: true,
    ...
    /*
     *   Get lesson plan details by Id
     */
    async findById(id: number): Promise<LessonPlans> {
        const divisionSubject = await this.lessonPlansRepository.findOne({
            where: { id },
    ...
    async customFindBuQuery(query) {
        const lessonPlan = await this.lessonPlansRepository.find(query);

        return lessonPlan;
    }

    async findUniqTeacher(query: any) {
        const lessonPlan = await this.lessonPlansRepository.find({
            where: {
                ...query.where,
    ...
    async delete(id: number) {
        ...
        return await this.lessonPlansRepository.save(lessonPlan);
    }

    /*
     *   Update lesson plan by id
     */
    async update(id: number, updateLessonPlanDto: UpdateLessonPlanDto): Promise<LessonPlans> {
        const updatedLessonPlan = await this.lessonPlansRepository.findOne({ where: { id } });
        Object.assign(updatedLessonPlan, updateLessonPlanDto);
        await this.lessonPlansRepository.save(updatedLessonPlan);
    ...
    async addStudent(lessonPlanId, studentIds) {
        const lessonPlanRepository = this.lessonPlansRepository;
        ...
        // Add students to the lesson plan
